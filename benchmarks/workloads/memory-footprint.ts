...
export function measureSmallRowFootprint(rows = 100_000): MemoryFootprint {
  ...
  db.exec("CREATE TABLE memory_rows (n int, label text)");
  const insert = db.prepare("INSERT INTO memory_rows(n, label) VALUES ($1, $2)");
  db.transaction(() => {
  ...
  if (Number(count) !== rows) throw new Error(`memory setup inserted ${count ?? 0} of ${rows} rows`);
  maybeGc();
