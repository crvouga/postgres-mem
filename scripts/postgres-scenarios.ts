 ...
 * Run: bun run scripts/postgres-scenarios.ts
 *      bun run scenarios
 */
    ...
    for (const scenario of section.scenarios) {
      ...
      for (const rel of scenario.evidence) {
        ...
        try {
          ...
          // Directory evidence: presence is enough for non-promoted sections.
        }
      }
      if (!foundInEvidence) {
        if (section.promoted) failures.push(`Promoted ${scenario.id} not found in evidence files`);
      } else {
        mapped += 1;
      }
    }
