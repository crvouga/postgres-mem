...
function parseArgs(argv: string[]): { seed?: number; depth: number; runs: number; path?: string } {
  ...
  let depth = Number(process.env.POSTGRES_MEM_WALK_STEPS ?? "200");
  let runs = Number(process.env.POSTGRES_MEM_FUZZ_RUNS ?? "20");
...
const details = await fc.check(
  fc.asyncProperty(decisionVectorArb(args.depth), async (ints) => {
    await runWalkOrMinimize(ints, { depth: args.depth, label: "soak" });
  }),
  {
    seed,
...
if (details.failed) {
  ...
  console.error(
    `Replay: POSTGRES_MEM_FUZZ_SEED=${details.seed} POSTGRES_MEM_WALK_STEPS=${args.depth}` +
      (details.counterexamplePath ? ` POSTGRES_MEM_FUZZ_PATH='${details.counterexamplePath}'` : "") +
